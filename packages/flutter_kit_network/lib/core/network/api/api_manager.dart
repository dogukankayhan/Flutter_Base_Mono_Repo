...
class DioApiManager implements ApiManager {
  ...
  Future<ApiResponse<R>> sendRequest<T, R>(
    ...
    try {
      ...
      final response = requestQueue != null
          ? await requestQueue!.enqueue(
              () => _executeRequest(method: httpMethod, path: path, body: data, query: queryParameters, headers: headers, cancelToken: cancelToken),
              priority: priority,
            )
          : await _executeRequest(method: httpMethod, path: path, body: data, query: queryParameters, headers: headers, cancelToken: cancelToken);

      final R parsed;
      if (extractor != null) {
        parsed = extractor(response.data);
      } else if (<T>[] is R) {
        parsed = _bodyParser<T>(response.data, fromJson, listWrapperKey: listWrapperKey) as R;
      } else {
